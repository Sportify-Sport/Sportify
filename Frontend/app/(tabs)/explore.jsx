...
export default function ExploreScreen() {
    ...
    try {
      ...
      if (data.success && data.result && data.result.records) {
        const record = data.result.records.find(
          (r) => r._id.toString() === cityId.toString()
        );
      ...
      console.error(
        "getCityNameById: Error fetching city from gov API:",
        error
      );
      return null;
    }
  ...
  const loadGroups = async (reset = false) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/Groups/GetGroups${query}`, {
        headers,
      });
      if (!response.ok) {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        setGroups((prev) =>
          reset ? enhancedGroups : [...prev, ...enhancedGroups]
        );
  ...
  const loadEvents = async (reset = false) => {
    ...
    try {
      ...
      if (!reset && lastEventId && lastEventDate) {
        query += `&lastEventId=${lastEventId}&lastEventDate=${encodeURIComponent(
          lastEventDate
        )}`;
      }

      const response = await fetch(`${apiUrl}/api/Events/GetEvents${query}`, {
        headers,
      });
      if (!response.ok) {
        if (response.status === 401 && !token) {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        setEvents((prev) =>
          reset ? enhancedEvents : [...prev, ...enhancedEvents]
        );
  ...
  const renderGroup = ({ item }) => (
    ...
    >
      ...
      <View className="ml-4 flex-1">
        ...
        <View className="flex-row items-center mt-1">
          <Ionicons
            name={getSportIcon(item.sportId)}
            size={16}
            color="#4b5563"
          />
          <Text className="text-sm text-gray-700 ml-1">
  ...
  return (
    <View className="flex-1 bg-gray-100 p-4">
      <TouchableOpacity
        onPress={() =>
          router.push({ pathname: "../screens/Search", params: { type } })
        }
      ...
      <View className="flex-row justify-center mb-4">
        ...
        >
          <Text
            className={`text-lg font-bold ${
              type === "event" ? "text-white" : "text-gray-700"
            }`}
          >
            Events
          </Text>
        ...
        >
          <Text
            className={`text-lg font-bold ${
              type === "group" ? "text-white" : "text-gray-700"
            }`}
          >
            Groups
          </Text>
