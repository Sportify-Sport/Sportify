...
//       <Image source={{ uri: `${apiUrl}/Images/${item.profileImage}` }} className="w-full h-48 rounded-lg" />
  ...
  return (
    <ScrollView className="flex-1 bg-white p-4">
      ...
      <View className="mb-6">
        <TouchableOpacity
          className="flex-row items-center mb-2"
          onPress={() => token && router.replace('/screens/EventDetails')}
          disabled={!token}
        >
          <Text className={`text-xl font-semibold ${token ? '' : 'text-gray-400'}`}>My Events</Text>
          <Text className={`${token ? 'text-[#65DA84]' : 'text-gray-400'} ml-1`}>›</Text>
      ...
      <View className="mb-6">
        <TouchableOpacity
          className="flex-row items-center mb-2"
          onPress={() => token && router.replace('/screens/GroupDetails')}
          disabled={!token}
        >
          <Text className={`text-xl font-semibold ${token ? '' : 'text-gray-400'}`}>My Groups</Text>
          <Text className={`${token ? 'text-[#65DA84]' : 'text-gray-400'} ml-1`}>›</Text>
