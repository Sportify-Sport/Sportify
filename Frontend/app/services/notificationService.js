...
class NotificationService {
  ...
  async initialize() {
    try {
      ...
      try {
        ...
        console.error('💡 Make sure you\'re using EAS Development Build, not Expo Go');
        return null;
      }
    } catch (error) {
  ...
  async registerTokenWithServer(token, authToken) {
    try {
      ...
      const response = await fetch(`${this.apiUrl}/api/notification/register-token`, {
        method: 'POST',
        headers: {
  ...
  async updateTokenIfNeeded(authToken) {
    try {
      ...
      const shouldUpdate = this.shouldRefreshToken(lastRegistration, lastStoredToken);
      ...
      if (shouldUpdate) {
        ...
        try {
          ...
          if (currentToken && currentToken !== this.token) {
            ...
            await this.registerTokenWithServer(currentToken, authToken);
  ...
  shouldRefreshToken(lastRegistration, lastStoredToken) {
    ...
    const hoursSinceLastRegistration = (new Date() - new Date(lastRegistration)) / (1000 * 60 * 60);
  ...
  async sendAdminNotification(message, eventId = null, groupId = null, recipients = 'all', authToken) {
    try {
      const response = await fetch(`${this.apiUrl}/api/notification/send-admin-notification`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`,
  ...
  async getNotificationHistory(authToken) {
    try {
      const response = await fetch(`${this.apiUrl}/api/notification/history`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
        },
      });
  ...
  async markNotificationAsRead(notificationId, authToken) {
    try {
      const response = await fetch(`${this.apiUrl}/api/notification/mark-read/${notificationId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
        },
